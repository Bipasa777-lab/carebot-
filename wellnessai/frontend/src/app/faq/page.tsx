      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="space-y-6">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/30 cursor-pointer"
              onClick={() => toggleFAQ(index)}
            >
              <h3 className="text-2xl font-semibold text-[#0c0966] flex justify-between items-center [font-family:'Outfit',Helvetica]">
                {faq.question}
              ...
              {openIndex === index && (
                <p className="text-lg text-black leading-relaxed mt-3 [font-family:'Outfit',Helvetica]">
                  {faq.answer}
                </p>
              )}
            </div>
